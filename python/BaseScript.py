...
class SampleObjectsAction:
	...
	def onUpdate(self, RC, scene, dt):
		...
		if len(selection) > 0:
			...
			RC.goTo(CommandReturnAction.DelAndForward).to(selection[0].position+vec4(0,0,1.5,0)).orientation(vec3(0,0,-1)).name('GoTo 1').insert(RC,2)
			RC.goTo(CommandReturnAction.DelAndForward).to(selection[0].position).orientation(vec3(0,0,-1)).name('GoTo 2').insert(RC,3)
			RC.grab(selection[0], CommandReturnAction.DelAndForward).insert(RC, 4)
			RC.goTo(CommandReturnAction.DelAndForward).to(getCursor()+vec4(0,0,0.2,0)).orientation(vec3(0,0,-1)).name('GoTo 3').insert(RC,5)
			RC.release(CommandReturnAction.DelAndBack).insert(RC,6)
