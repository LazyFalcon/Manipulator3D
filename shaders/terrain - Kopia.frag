...
void main(void){
	...
	float layer = ceil(slope*4);
	float layer2 = layer + 1;
	
	vec4 color = texture2DArray(grounds, vec3(uv,layer));
	vec4 color2 = texture2DArray(grounds, vec3(uv,layer2));
	// color = mix(color, color2, slope*4);
	color = heightBlend(color, color2, fract(slope*4), 0.1);
	// color = vec4(1)*slope;
	...
	// color = t1g;
	color = clamp(sunLight*color*sun_color*3.3 + spec*sun_color*dot(color,color), 0.0, 1.0);
