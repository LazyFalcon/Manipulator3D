...
const float bias = 0.532069;
const float scale = 170.01;
// const float intensity = 3.1;
const float intensity = 28.1;
// const vec2 sampleRadius = 175/vec2(1400, 720);
const vec2 sampleRadius = vec2(0.0315);
...
vec4 getPosition(vec2 uv, out float depth){
	...
	// vec4 viewSpace = vec4(uv, depth, 1);
...
void main(void){
	...
	// ao *= ao;
	ao = sqrt(clamp(ao/float(iterations)*0.7, 0, 1));
	// ao = sqrt(ao);
	// outAO = 1.01-ao;
	outAO = vec4(1.01-ao);
}
